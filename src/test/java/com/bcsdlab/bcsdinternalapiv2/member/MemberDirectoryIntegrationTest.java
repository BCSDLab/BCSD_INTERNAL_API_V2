...
class MemberDirectoryIntegrationTest {
    ...
    private TrackMaster game;
    private TrackMaster security;
    ...
    void setUp() {
        ...
        game = trackMasterRepository.findByCode("GAME").orElseThrow();
        security = trackMasterRepository.findByCode("SECURITY").orElseThrow();
    ...
    void 트랙과_구분으로_필터링하면_조건에_맞는_회원만_반환된다() throws Exception {
        ...
        saveMember("20240001", "백엔드멘토", backend, MemberType.MENTOR);
        saveMember("20240002", "프론트레귤러", frontend, MemberType.REGULAR);
        saveMember("20240003", "게임비기너", game, MemberType.BEGINNER);

        mockMvc.perform(get("/v1/admin/members")
                        .header("Authorization", "Bearer " + adminToken)
                        .param("keyword", "2024000")
                        .param("track", "BACKEND")
    ...
    @Test
    void SECURITY_트랙_회원도_조회된다() throws Exception {
        String adminToken = createAdminAndLogin("20230002");
        saveMember("20240011", "보안담당", security, MemberType.REGULAR);

        mockMvc.perform(get("/v1/admin/members")
                        .header("Authorization", "Bearer " + adminToken)
                        .param("track", "SECURITY"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.members.length()").value(1))
                .andExpect(jsonPath("$.members[0].track").value("SECURITY"));
    }

    @Test
    void 이름_학번_이메일_깃허브아이디로_통합검색이_동작한다() throws Exception {
        String adminToken = createAdminAndLogin("20230003");
        saveMember("20240021", "김검색", backend, MemberType.REGULAR);
        saveMember("20240022", "박다름", backend, MemberType.REGULAR);

        mockMvc.perform(get("/v1/admin/members")
                        .header("Authorization", "Bearer " + adminToken)
                        .param("keyword", "20240021"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.members.length()").value(1))
    ...
    void 기수_오름차순_정렬이_적용된다() throws Exception {
        ...
        saveMemberWithGeneration("20240031", "24-하");
        saveMemberWithGeneration("20240032", "20-상");
        saveMemberWithGeneration("20240033", "22-하");
    ...
    void 페이지네이션_경계값에서_올바른_페이지수와_총원이_반환된다() throws Exception {
        ...
        for (int i = 0; i < 9; i++) {
            saveMemberWithGeneration("2024040" + i, "24-하");
        }

        mockMvc.perform(get("/v1/admin/members")
                        .header("Authorization", "Bearer " + adminToken)
                        .param("keyword", "202404")
                        .param("page", "0"))
                .andExpect(status().isOk())
    ...
    void 활동_여부와_학적_상태_필터가_동시에_적용된다() throws Exception {
        ...
        saveMemberWithStatus("20240051", AcademicStatus.GRADUATED, false);
        saveMemberWithStatus("20240052", AcademicStatus.GRADUATED, true);
        saveMemberWithStatus("20240053", AcademicStatus.ENROLLED, false);

        mockMvc.perform(get("/v1/admin/members")
                        .header("Authorization", "Bearer " + adminToken)
                        .param("keyword", "2024005")
                        .param("active", "false")
                        .param("academicStatus", "GRADUATED"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.members.length()").value(1))
                .andExpect(jsonPath("$.members[0].studentNumber").value("20240051"));
    }

    @Test
    void 사이드바_카운트는_필터와_무관하게_전체_기준으로_집계된다() throws Exception {
        String adminToken = createAdminAndLogin("20230007");
        saveMember("20240061", "백엔드1", backend, MemberType.REGULAR);
        saveMember("20240062", "프론트1", frontend, MemberType.REGULAR);
        saveMember("20240063", "게임1", game, MemberType.REGULAR);

        mockMvc.perform(get("/v1/admin/members")
                        .header("Authorization", "Bearer " + adminToken)
                        .param("keyword", "2024006")
                        .param("track", "BACKEND"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.members.length()").value(1))
    ...
    void 관리자는_학적_상태와_활동_여부를_변경할_수_있다() throws Exception {
        String adminToken = createAdminAndLogin("20230008");
        Member target = saveMember("20240071", "상태변경대상", backend, MemberType.REGULAR);
        ...
        assertThat(updated.getAcademicStatus()).isEqualTo(AcademicStatus.GRADUATED);
        assertThat(updated.isClubActive()).isFalse();
    }

    @Test
    void 일반_회원_권한으로는_학적_상태_및_활동여부를_변경할_수_없다() throws Exception {
        String memberToken = createMemberAndLogin("20230009");
        Member target = saveMember("20240081", "권한없음대상", backend, MemberType.REGULAR);
    ...
    void 일반_회원도_인명부_조회_API로_전화번호_학번을_포함한_전체_정보를_조회할_수_있다() throws Exception {
        String memberToken = createMemberAndLogin("20230010");
        saveMember("20240091", "조회대상", backend, MemberType.REGULAR);

        mockMvc.perform(get("/v1/members/directory")
                        .header("Authorization", "Bearer " + memberToken)
                        .param("keyword", "20240091"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.members[0].studentNumber").value("20240091"))
                .andExpect(jsonPath("$.members[0].phoneNumber").value("010240091"));
    }
    ...
    void 관리자는_회원_프로필을_수정할_수_있다() throws Exception {
        ...
        Member target = saveMember("20240101", "수정전이름", backend, MemberType.REGULAR);
    ...
    void 관리자는_회원_권한을_변경할_수_있다() throws Exception {
        ...
        Member target = saveMember("20240102", "권한변경대상", backend, MemberType.REGULAR);
    ...
    void 관리자는_회원을_탈퇴_처리하고_복구할_수_있다() throws Exception {
        String adminToken = createAdminAndLogin("20230013");
        Member target = saveMember("20240103", "탈퇴대상", backend, MemberType.REGULAR);
    ...
    void 일반_회원_권한으로는_프로필_수정_권한변경_탈퇴처리를_할_수_없다() throws Exception {
        String memberToken = createMemberAndLogin("20230014");
        Member target = saveMember("20240104", "권한없음대상2", backend, MemberType.REGULAR);
    ...
    void 관리자는_사진_업로드용_presigned_URL을_발급받을_수_있다() throws Exception {
        ...
        when(presignedRequest.url())
                .thenReturn(URI.create("https://image.bcsdlab.com/member-photos/test").toURL());
        when(s3Presigner.presignPutObject(any(PutObjectPresignRequest.class))).thenReturn(presignedRequest);

        String adminToken = createAdminAndLogin("20230015");
        Member target = saveMember("20240105", "사진대상", backend, MemberType.REGULAR);
    ...
    void 관리자는_업로드된_사진_URL을_회원_프로필에_등록할_수_있다() throws Exception {
        String adminToken = createAdminAndLogin("20230016");
        Member target = saveMember("20240106", "사진등록대상", backend, MemberType.REGULAR);
        ...
        assertThat(memberRepository.findById(target.getId()).orElseThrow().getProfileImageUrl())
    ...
    private Member saveMember(String studentNumber, String name, TrackMaster track, MemberType memberType) {
        return saveMemberFull(studentNumber, name, track, memberType, "24-하", AcademicStatus.ENROLLED, true);
    }

    private Member saveMemberWithGeneration(String studentNumber, String generation) {
        return saveMemberFull(studentNumber, "기수테스트", backend, MemberType.REGULAR, generation,
    ...
    private Member saveMemberWithStatus(String studentNumber, AcademicStatus academicStatus, boolean active) {
        return saveMemberFull(studentNumber, "상태테스트", backend, MemberType.REGULAR, "24-하",
                academicStatus, active);
    }

    private Member saveMemberFull(String studentNumber, String name, TrackMaster track, MemberType memberType,
